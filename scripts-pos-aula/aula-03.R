...
concentracoes_urbanas <- read_urban_concentrations()

glimpse(concentracoes_urbanas)

head(concentracoes_urbanas)

concentracoes_urbanas |> 
  ggplot() +
  geom_sf()

concentracoes_urbanas |> 
  ggplot() +
  geom_sf(aes(fill = pop_rural_2010))
...
pnud_uf_sf |> 
 filter(ano == 2010) |> 
  ggplot()+
  geom_sf(aes(fill = espvida), color = "pink") +
  theme_light() +
  scale_fill_gradient(low = "red",
                      high = "green") # +
  #facet_wrap(~ ano)

# facet nunca da bom ou foi nesse caso?
...
ggplot() +
  geom_sf(data = municipio_brasilia) +
  geom_sf(data = escolas_brasilia, aes(color = government_level)) +
  facet_wrap(~government_level, nrow = 3)
...
pnud_poligonos <- municipios |> 
  left_join(dados_pnud_2010, by = c("code_muni" = "muni_id"))

MAPA_RO <- pnud_poligonos |> 
  filter(uf_sigla == "RO") |> 
  ggplot() +
  geom_sf(aes(fill = espvida, color = espvida)) +
  theme_bw() +
...
ggsave(plot = MAPA_RO,
       filename = "output/mapa_ro.png")


# Regiões metropolitanas -------
geobr::list_geobr() 

regioes_metropolitanas <- read_metro_area()


regioes_metropolitanas |> 
  filter(abbrev_state == "SP") |> 
  ggplot() +
  geom_sf(aes(fill = name_metro))
