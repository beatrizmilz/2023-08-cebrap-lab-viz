...
mananciais |> 
  ...
  ggplot() +
  aes(y = volume_porcentagem, x = sistema) +
  geom_col()
...
mananciais |> 
  filter(data == max(data)) |> 
  ggplot() +
  aes(x = volume_porcentagem, y = sistema) +
  geom_col()
...
mananciais |> 
  filter(data == max(data)) |> 
  ggplot() +
  aes(x = volume_operacional, y = sistema) +
  geom_col()
...
barragens |> 
  ggplot() +
  aes(y = altura_atual_m, x = categoria_de_risco) +
  geom_boxplot()


# 05. Crie um gráfico de dispersão, onde o eixo x representa a altura 
# atual das barragens,
...
options(scipen = 999) # nao deixa a notacao
options(scipen = 1)

barragens |> 
  ggplot() +
  aes(x = altura_atual_m, y = volume_m3) +
  geom_point(aes(color = dano_potencial_associado), alpha = 0.5) +
  scale_y_log10()


# outra forma
barragens |> 
  mutate(volume_milhao = volume_m3 / 1e6) |> 
  ggplot() +
  aes(x = altura_atual_m, y = volume_m3) +
  geom_point(aes(color = dano_potencial_associado), alpha = 0.5)
...
library(janitor)
library(tidyverse)
Relatorio_20234030 <- read_excel("~/Downloads/Relatorio_20234030.xlsx", 
                                 skip = 1)

barragens <- clean_names(Relatorio_20234030) |>
  mutate(# transformar a coluna volume_atual_m3 em número
    # por padrão ela está como texto.
